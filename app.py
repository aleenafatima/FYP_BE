...
## 'app.py' with speed, swing, and spin functions
...
# def calculate_spin(points):
#     if len(points) < 3:
#         return None  # Not enough data points for calculating spin
    
#     y_coords = [point[1] for point in points]
#     peaks, _ = find_peaks(y_coords)
    
#     if len(peaks) < 2:
#         return None  # Not enough peaks in trajectory for calculating spin
    
#     first_peak = peaks[0]
#     second_peak = peaks[1]
    
#     first_peak_x = points[first_peak][0]
#     second_peak_x = points[second_peak][0]
    
#     spin = abs(second_peak_x - first_peak_x)
#     return spin
...
#         points, frame_width = track_ball(filepath, output_path)
#         swing = calculate_swing(points, frame_width)
#         spin = calculate_spin(points)
#         speed_metrics = f"{random_speed_kph:.2f} kph ({random_speed_mph:.2f} mph)"
#         swing_metrics = f"Swing: {swing:.2f} cm"
#         if spin is not None:
#             spin_metrics = f"Spin: {spin:.2f} pixels"
#         else:
...
def calculate_spin(points, frame_width):
    ...
    spin_pixels = abs(second_peak_x - first_peak_x)
    
    # Convert spin from pixels to cm
    wicket_width_cm = 61  # Width of the wicket in cm
    pixels_per_cm = frame_width / wicket_width_cm
    spin_cm = spin_pixels / pixels_per_cm
